...
void read_1_info(User& A, ifstream& f) {
	string temp;
	getline(f, temp, ',');
	A.info.NO_inclass = stoi(temp);
	getline(f, A.info.IDstd, ',');
...
void read_info(User& A, SchoolYear SY) {
	string path = "database/HCMUS/" + SY.year + '/';
	string fileName;
	if (A.role == 1) {
		fileName = path + A.info.Class + csv_tail;
	}
	else {
		fileName = path + "staff.csv";
	}
	ifstream info_file;
...
void read_course(User A, SchoolYear y)
{
	...
	gotoxy(1, 15); cout << "N.o";
	gotoxy(8, 15); cout << "Id course";
	gotoxy(19, 15); cout << "Course name";
	gotoxy(45, 15); cout << "Lecturer";
	gotoxy(70, 15); cout << "Number of credits";
	...
	while (tempo != NULL)
	{
		...
		while (!fi.eof())
		{
			...
			if (_strcmpi(co.ID_course.c_str(), tempo->data.ID.c_str()) == 0)
			{
				...
				gotoxy(1, 15 + i); cout << i;
				gotoxy(8, 15 + i); cout << co.ID_course;
				gotoxy(19, 15 + i); cout << co.name;
				gotoxy(45, 15 + i); cout << co.teacher;
				gotoxy(78, 15 + i); cout << co.Num_of_creadit;
				gotoxy(94, 15 + i); cout << co.Max_student;
				gotoxy(102, 15 + i); cout << co.Schedule;
				i++;
				tempo = tempo->pNext;
				break;
			}
...
void init_List_Mark(MarkNode*& head){ head = NULL;}
...
Data* read_file_student_info_of_course(SchoolYear SY, string IDcourse, int& n) {
	...
	getline(f, temp);
	for (int i = 0; i < n; i++) {
		getline(f, info[i].IDstd, ',');
		getline(f, info[i].name, ',');
		getline(f, info[i].Bir, ',');
		getline(f, info[i].sex, ',');
		getline(f, info[i].IDsocial);
		info[i].NO_inclass = i + 1;
	}
	f.close();
	return info;
}
